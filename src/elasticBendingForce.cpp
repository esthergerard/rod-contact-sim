...
elasticBendingForce::elasticBendingForce(elasticRod &m_rod, timeStepper &m_stepper, int idx)
{
    rod = &m_rod;
    stepper = &m_stepper;
...
void elasticBendingForce::computeFb()
{
    ...
    for (int i = 1; i < rod->ne; i++)
    {
        ...
        for (int k = 0; k < 11; k++)
        {
            ...
            stepper->addForce(ind, -f[k], rod_idx); // subtracting elastic force
        }
    }
}
...
void elasticBendingForce::computeJb()
{
    for (int i = 1; i < rod->ne; i++)
    {
        ...
        tt_o_tt = tilde_t * tilde_t.adjoint();

        crossMat(tilde_d1, tilde_d1_3d);
        crossMat(tilde_d2, tilde_d2_3d);

        tmp = tf.cross(tilde_d2);
        tf_c_d2t_o_tt = tmp * tilde_t.adjoint();
        tt_o_tf_c_d2t = tf_c_d2t_o_tt.adjoint();
        kb_o_d2e = kbLocal * d2e.adjoint();
        d2e_o_kb = kb_o_d2e.adjoint();

        D2kappa1De2 =
            1.0 / norm2_e * (2 * kappa1 * tt_o_tt - tf_c_d2t_o_tt - tt_o_tf_c_d2t) - kappa1 / (chi * norm2_e) * (Id3 - te * te.adjoint()) + 1.0 / (4.0 * norm2_e) * (kb_o_d2e + d2e_o_kb);

        tmp = te.cross(tilde_d2);
        te_c_d2t_o_tt = tmp * tilde_t.adjoint();
        tt_o_te_c_d2t = te_c_d2t_o_tt.adjoint();
        kb_o_d2f = kbLocal * d2f.adjoint();
        d2f_o_kb = kb_o_d2f.adjoint();

        D2kappa1Df2 =
            1.0 / norm2_f * (2 * kappa1 * tt_o_tt + te_c_d2t_o_tt + tt_o_te_c_d2t) - kappa1 / (chi * norm2_f) * (Id3 - tf * tf.adjoint()) + 1.0 / (4.0 * norm2_f) * (kb_o_d2f + d2f_o_kb);

        D2kappa1DeDf =
            -kappa1 / (chi * norm_e * norm_f) * (Id3 + te * tf.adjoint()) + 1.0 / (norm_e * norm_f) * (2 * kappa1 * tt_o_tt - tf_c_d2t_o_tt + tt_o_te_c_d2t - tilde_d2_3d);
        D2kappa1DfDe = D2kappa1DeDf.adjoint();

        tmp = tf.cross(tilde_d1);
        tf_c_d1t_o_tt = tmp * tilde_t.adjoint();
        tt_o_tf_c_d1t = tf_c_d1t_o_tt.adjoint();
        kb_o_d1e = kbLocal * d1e.adjoint();
        d1e_o_kb = kb_o_d1e.adjoint();

        D2kappa2De2 = 1.0 / norm2_e * (2 * kappa2 * tt_o_tt + tf_c_d1t_o_tt + tt_o_tf_c_d1t) - kappa2 / (chi * norm2_e) * (Id3 - te * te.adjoint()) - 1.0 / (4.0 * norm2_e) * (kb_o_d1e + d1e_o_kb);

        tmp = te.cross(tilde_d1);
        Matrix3d te_c_d1t_o_tt = tmp * tilde_t.adjoint();
        Matrix3d tt_o_te_c_d1t = te_c_d1t_o_tt.adjoint();
        Matrix3d kb_o_d1f = kbLocal * d1f.adjoint();
        Matrix3d d1f_o_kb = kb_o_d1f.adjoint();

        D2kappa2Df2 = 1.0 / norm2_f * (2 * kappa2 * tt_o_tt - te_c_d1t_o_tt - tt_o_te_c_d1t) - kappa2 / (chi * norm2_f) * (Id3 - tf * tf.adjoint()) - 1.0 / (4.0 * norm2_f) * (kb_o_d1f + d1f_o_kb);

        D2kappa2DeDf = -kappa2 / (chi * norm_e * norm_f) * (Id3 + te * tf.adjoint()) + 1.0 / (norm_e * norm_f) * (2 * kappa2 * tt_o_tt + tf_c_d1t_o_tt - tt_o_te_c_d1t + tilde_d1_3d);

        D2kappa2DfDe = D2kappa2DeDf.adjoint();
        ...
        D2kappa1DeDthetae = 1.0 / norm_e * ((0.5 * kbLocal.dot(d1e)) * tilde_t - 1.0 / chi * (tf.cross(d1e)));
        D2kappa1DeDthetaf = 1.0 / norm_e * ((0.5 * kbLocal.dot(d1f)) * tilde_t - 1.0 / chi * (tf.cross(d1f)));
        D2kappa1DfDthetae = 1.0 / norm_f * ((0.5 * kbLocal.dot(d1e)) * tilde_t + 1.0 / chi * (te.cross(d1e)));
        D2kappa1DfDthetaf = 1.0 / norm_f * ((0.5 * kbLocal.dot(d1f)) * tilde_t + 1.0 / chi * (te.cross(d1f)));
        D2kappa2DeDthetae = 1.0 / norm_e * ((0.5 * kbLocal.dot(d2e)) * tilde_t - 1.0 / chi * (tf.cross(d2e)));
        D2kappa2DeDthetaf = 1.0 / norm_e * ((0.5 * kbLocal.dot(d2f)) * tilde_t - 1.0 / chi * (tf.cross(d2f)));
        D2kappa2DfDthetae = 1.0 / norm_f * ((0.5 * kbLocal.dot(d2e)) * tilde_t + 1.0 / chi * (te.cross(d2e)));
        D2kappa2DfDthetaf = 1.0 / norm_f * ((0.5 * kbLocal.dot(d2f)) * tilde_t + 1.0 / chi * (te.cross(d2f)));
        ...
        DDkappa1.row(3).segment(0, 3) = DDkappa1.col(3).segment(0, 3).adjoint();
        DDkappa1.row(3).segment(4, 3) = DDkappa1.col(3).segment(4, 3).adjoint();
        DDkappa1.row(3).segment(8, 3) = DDkappa1.col(3).segment(8, 3).adjoint();
        ...
        DDkappa1.row(7).segment(0, 3) = DDkappa1.col(7).segment(0, 3).adjoint();
        DDkappa1.row(7).segment(4, 3) = DDkappa1.col(7).segment(4, 3).adjoint();
        DDkappa1.row(7).segment(8, 3) = DDkappa1.col(7).segment(8, 3).adjoint();
        ...
        DDkappa2.row(3).segment(0, 3) = DDkappa2.col(3).segment(0, 3).adjoint();
        DDkappa2.row(3).segment(4, 3) = DDkappa2.col(3).segment(4, 3).adjoint();
        DDkappa2.row(3).segment(8, 3) = DDkappa2.col(3).segment(8, 3).adjoint();
        ...
        DDkappa2.row(7).segment(0, 3) = DDkappa2.col(7).segment(0, 3).adjoint();
        DDkappa2.row(7).segment(4, 3) = DDkappa2.col(7).segment(4, 3).adjoint();
        DDkappa2.row(7).segment(8, 3) = DDkappa2.col(7).segment(8, 3).adjoint();

        len = rod->voronoiLen(i);
        relevantPart.col(0) = gradKappa1.row(i);
        relevantPart.col(1) = gradKappa2.row(i);

        Jbb = -1.0 / len * relevantPart * EIMat * relevantPart.adjoint();

        kappaL = (rod->kappa).row(i) - (rod->kappaBar).row(i);

        temp = -1.0 / len * kappaL.adjoint() * EIMat;
        ...
        for (int j = 0; j < 11; j++)
        {
            for (int k = 0; k < 11; k++)
            {
                int ind1 = 4 * i - 4 + j;
                int ind2 = 4 * i - 4 + k;
                stepper->addJacobian(ind1, ind2, -Jbb(k, j), rod_idx);
            }
        }
    }
}

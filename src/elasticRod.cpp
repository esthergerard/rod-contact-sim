...
{
	return (Vector3d(x(4 * k), x(4 * k + 1), x(4 * k + 2)) -
			Vector3d(x0(4 * k), x0(4 * k + 1), x0(4 * k + 2))) /
		   dt;
}
...
void elasticRod::updateNewtonX(double *dx, int i, double a)
{
	int offset = i * uncons;
	for (int c = 0; c < uncons; c++)
	{
		x[unconstrainedMap[c]] -= a * dx[c + offset];
	}
}
...
{
	// We cannot have perfectly 0 movement if using friction.
	if (friction)
	{
		for (int c = 0; c < uncons; c++)
		{
			//		    x[unconstrainedMap[c]] += 0.01 * u[unconstrainedMap[c]] * dt;
			//            x[unconstrainedMap[c]] +=  0.25 * u[unconstrainedMap[c]] * dt;
		}
	}
}

// for Regularized Stokeslet method

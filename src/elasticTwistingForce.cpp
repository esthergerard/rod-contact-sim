...
elasticTwistingForce::elasticTwistingForce(elasticRod &m_rod, timeStepper &m_stepper, int idx)
{
    rod = &m_rod;
    stepper = &m_stepper;
...
void elasticTwistingForce::computeJt()
{
    for (int i = 1; i < rod->ne; i++)
    {
        ...
        D2mDe2 = -0.25 / norm2_e * (kbLocal * (te + tilde_t).adjoint() + (te + tilde_t) * kbLocal.adjoint());
        D2mDf2 = -0.25 / norm2_f * (kbLocal * (tf + tilde_t).adjoint() + (tf + tilde_t) * kbLocal.adjoint());
        D2mDeDf = 0.5 / (norm_e * norm_f) * (2.0 / chi * teMatrix - kbLocal * tilde_t.adjoint());
        ...
        Jtt = GJ * milen * ((deltam(i) + rod->refTwist(i) - rod->undeformedTwist(i)) * DDtwist + gradTwistLocal * gradTwistLocal.adjoint());
        ...
        for (int j = 0; j < 11; j++)
        {
            for (int k = 0; k < 11; k++)
            {
                ind1 = 4 * i - 4 + j;
                ind2 = 4 * i - 4 + k;
                stepper->addJacobian(ind1, ind2, -Jtt(k, j), rod_idx);
            }
        }
    }
}
